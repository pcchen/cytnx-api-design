...
| `Tensor_05_arithmetic.cpp` | **T-A4** in-place `Conj_`/`Abs_`/`Exp_`/`Pow_`/`Inv_` each return `Tensor&` (`&*this`) — the conti.py `return self` shims / leaked `c*` bindings are binding plumbing · **T-A2** `linalg::Dot` computes the matrix product and `A = Dot(A,S)` (the raw `c__imatmul__` primitive body) mutates in place — so `@=`'s brokenness is the misnamed conti.py `__imatmul` wrapper (B-5), not a C++/primitive gap |
