...
| `void to_(const int&)` (`:683`) | `to_` (`py:177`, `&Tensor::to_`, `py::arg("device")`) | **signature-differs** | C++ returns `void` → Python `to_` returns **None**, not self (T-T4); param correctly named `device` (T-T5, the positive contrast to UT-T3) |
