...
| `Arnoldi` | 2 overloads: `Arnoldi(Hop, Tin, which='LM', Maxiter=10000, CvgCrit=0, k=1, is_V=True, ncv=0, verbose=False)` in a **Tensor** and a **UniTensor** operand variant | `list[Tensor]` / `list[UniTensor]` `[eigvals, (eigvecs)]` | General (non-Hermitian) Krylov eigensolver (ARPACK). `which` selects the target region (`'LM'`/`'SR'`/`'LR'`/`'LA'`; `'SM'` unsupported → `RuntimeError`). Probe: *"Arnoldi carries TWO overloads (Tensor and UniTensor)"* + *"Arnoldi(op, v, which='SR') runs and returns eigen-results"* + *"Arnoldi(op, v, which='SM') is unsupported and raises RuntimeError"*. |
...
- **`method`/`which` are *string* dispatch keys** (UT-K2): prefer an `enum`
  ...
  valid codes are otherwise undiscoverable and `'SM'` raises `RuntimeError`
  (probe: *"Arnoldi(op, v, which='SM') is unsupported and raises RuntimeError"*).
...
| `Lanczos_Gnd_Ut` | **remove → `LanczosGnd` (UniTensor overload)** (UT-K3/K4) | Currently **absent** (commented out). The UniTensor ground state is obtained via the `LanczosGnd` UniTensor overload (or, in 1.1.0, `Lanczos(Hop, ut_v0, method='Gnd')`). *Migration:* delete the dormant `c_Lanczos_Gnd_Ut` block; drop the `_Ut` suffix convention. Verified: probe *"Lanczos(Hop, ut_v0, method='Gnd') also runs on a UniTensor start vector and matches the same ground-state eigenvalue"*. |
...
(probe: *"Lanczos(Hop, ut_v0, method='Gnd') also runs on a UniTensor start
...
raised a `RuntimeError` in 1.1.0 (probe: *"Arnoldi(op, v, which='SM') is
